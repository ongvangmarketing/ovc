...
export async function getCustomerPortalData(session: any) {
  ...
  const [entitlements, organization, projects, quotations, contracts, invoices] = await Promise.all([
  ...
  const brand = {
    name: organizationSettings.company_workspace_name || organizationSettings.company_name || organization?.name || "OngVàng",
    favicon: organizationSettings.company_favicon_url || organization?.logo || "/brand/ong-vang-logo.svg",
  };
