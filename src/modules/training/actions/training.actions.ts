...
async function uniqueCourseSlug(organizationId: string, title: string, currentId?: string) {
  ...
  while (
    await getTenantDb().course.findFirst({
...
async function fallbackInstructorId(organizationId: string, userId: string) {
  const instructor = await getTenantDb().organizationMember.findFirst({
...
export async function createTrainingCourse(formData: FormData) {
  ...
  const course = await getTenantDb().course.create({
    data: {
...
export async function updateTrainingCourse(id: string, formData: FormData) {
  const session = await requireAuth();
  const current = await getTenantDb().course.findFirst({ where: { id, organizationId: session.organizationId } });
  ...
  await getTenantDb().course.update({
    where: { id },
    data: {
...
export async function createCourseSection(courseId: string, formData: FormData) {
  ...
  await getTenantDb().courseSection.create({
    data: {
      courseId,
...
export async function createCourseLesson(courseId: string, formData: FormData) {
  ...
  const section = await getTenantDb().courseSection.findFirst({
  ...
  await getTenantDb().lesson.create({
    data: {
      sectionId,
      title: text(formData, "title"),
      description: optionalText(formData, "description"),
...
export async function createLessonSchedule(courseId: string, lessonId: string, formData: FormData) {
  ...
  if (classId) {
    const classRecord = await getTenantDb().class.findFirst({ where: { id: classId, courseId } });
    if (!classRecord) throw new Error("Lớp học không hợp lệ");
  }

  const instructorId = text(formData, "instructorId") || null;
  if (instructorId) {
    const instructor = await getTenantDb().organizationMember.findFirst({ where: { organizationId: session.organizationId, userId: instructorId, user: { role: UserRole.INSTRUCTOR } } });
    if (!instructor) throw new Error("Giảng viên không hợp lệ");
  }

  await getTenantDb().$executeRaw`
...
export async function createTrainingClass(formData: FormData) {
  ...
  const course = await getTenantDb().course.findFirst({ where: { id: courseId, organizationId: session.organizationId }, select: { id: true } });
  if (!course) throw new Error("Khóa học không hợp lệ");

  const item = await getTenantDb().class.create({
    data: {
      courseId,
...
export async function updateTrainingClass(id: string, formData: FormData) {
  const session = await requireAuth();
  const current = await getTenantDb().class.findFirst({ where: { id, course: { organizationId: session.organizationId } }, include: { course: true } });
  if (!current) throw new Error("Không tìm thấy lớp học");
  const courseId = text(formData, "courseId", current.courseId);
  const course = await getTenantDb().course.findFirst({ where: { id: courseId, organizationId: session.organizationId }, select: { id: true } });
  if (!course) throw new Error("Khóa học không hợp lệ");

  await getTenantDb().class.update({
    where: { id },
    data: {
      courseId,
...
export async function createTrainingStudent(formData: FormData) {
  ...
  const user = await getTenantDb().user.upsert({
...
export async function createPotentialStudent(formData: FormData) {
  const session = await requireAuth();
  const item = await getTenantDb().potentialStudent.create({
    data: {
      organizationId: session.organizationId,
      name: text(formData, "name"),
...
export async function updatePotentialStudent(id: string, formData: FormData) {
  const session = await requireAuth();
  const current = await getTenantDb().potentialStudent.findFirst({ where: { id, organizationId: session.organizationId } });
  if (!current) throw new Error("Không tìm thấy học viên tiềm năng");

  await getTenantDb().potentialStudent.update({
    where: { id },
    data: {
      name: text(formData, "name", current.name),
      email: optionalText(formData, "email"),
...
export async function createTrainingTuition(formData: FormData) {
  ...
  const course = await getTenantDb().course.findFirst({ where: { id: courseId, organizationId: session.organizationId } });
  const member = await getTenantDb().organizationMember.findFirst({ where: { organizationId: session.organizationId, userId: studentId, user: { role: UserRole.STUDENT } } });
  if (!course || !member) throw new Error("Khóa học hoặc học viên không hợp lệ");

  const item = await getTenantDb().enrollment.upsert({
...
export async function updateTrainingTuition(id: string, formData: FormData) {
  const session = await requireAuth();
  const current = await getTenantDb().enrollment.findFirst({ where: { id, course: { organizationId: session.organizationId } } });
