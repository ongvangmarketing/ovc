...
export async function createDealAction(formData: FormData) {
  ...
  const deal = await getTenantDb().deal.create({
    data: {
...
export async function saveDealAction(payload: any) {
  ...
  if (id) {
    ...
    // Clear old options
    await getTenantDb().dealServiceOption.deleteMany({
      where: { dealId: id, organizationId: session.organizationId }
    });
  } else {
    // Create new deal
    const newDeal = await getTenantDb().deal.create({
      data: {
        ...data,
        organizationId: session.organizationId,
      }
  ...
  if (serviceOptions && serviceOptions.length > 0) {
    await getTenantDb().dealServiceOption.createMany({
...
export async function updateDealStageAction(dealId: string, stageId: string) {
  ...
  const stage = await getTenantDb().dealStage.findUnique({
...
export async function createDealStageAction(name: string, color: string = "#94A3B8") {
  ...
  const maxOrderStage = await getTenantDb().dealStage.findFirst({
  ...
  await getTenantDb().dealStage.create({
    data: {
      organizationId: session.organizationId,
      name,
      color,
...
export async function updateDealStageNameAction(id: string, name: string) {
  ...
  await getTenantDb().dealStage.update({
    where: { id, organizationId: session.organizationId },
    data: { name },
  });
...
export async function deleteDealStageAction(id: string) {
  ...
  const dealCount = await getTenantDb().deal.count({
  ...
  await getTenantDb().dealStage.delete({
    where: { id, organizationId: session.organizationId },
  });
...
export async function submitDealSelectionAction(dealId: string, selectedIds: string[], contactInfo?: { name?: string; phone?: string; email?: string; address?: string; taxCode?: string }, customerNote?: string) {
  ...
  if (selectedIds.length > 0) {
    await getTenantDb().dealServiceOption.updateMany({
  ...
  if (contactInfo) {
    ...
    if (!linkedCompanyId && contactInfo.name && normalizedTaxCode) {
      const existingCompany = normalizedEmail
        ? await getTenantDb().company.findFirst({ where: { organizationId: deal.organizationId, email: normalizedEmail } })
        : null;
      ...
      const company = existingCompany
        ? await getTenantDb().company.update({ where: { id: existingCompany.id }, data: companyData })
        : await getTenantDb().company.create({ data: { ...companyData, organizationId: deal.organizationId } });
    ...
    if (linkedContactId) {
      ...
      const existingContact = normalizedEmail
        ? await getTenantDb().contact.findFirst({ where: { organizationId: deal.organizationId, email: normalizedEmail } })
        : normalizedPhone
          ? await getTenantDb().contact.findFirst({ where: { organizationId: deal.organizationId, phone: normalizedPhone } })
          : null;
      ...
      const contact = existingContact
        ? await getTenantDb().contact.update({ where: { id: existingContact.id }, data: contactData })
        : await getTenantDb().contact.create({ data: { ...contactData, organizationId: deal.organizationId } });
      linkedContactId = contact.id;
    }

    if (linkedCompanyId && (normalizedTaxCode || normalizedAddress)) {
      const company = await getTenantDb().company.findUnique({ where: { id: linkedCompanyId } });
      if (company) {
  ...
  if (selectedIds.length > 0) {
    const lastStage = await getTenantDb().dealStage.findFirst({
