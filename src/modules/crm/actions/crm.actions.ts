...
export async function getContacts() {
  ...
  try {
    const contacts = await getTenantDb().contact.findMany({
...
export async function getDeals() {
  ...
  try {
    const deals = await getTenantDb().deal.findMany({
      where: { organizationId: session.organizationId },
      orderBy: { createdAt: "desc" },
      include: {
        company: true,
...
export async function getDealsPipeline() {
  ...
  try {
    ...
    const stageCount = await getTenantDb().dealStage.count({
      where: { organizationId: session.organizationId }
    });
    
    if (stageCount === 0) {
      await getTenantDb().dealStage.createMany({
        data: [
    ...
    const stages = await getTenantDb().dealStage.findMany({
      where: { organizationId: session.organizationId },
      orderBy: { order: 'asc' }
    });
    ...
    if (unassignedDeals.length > 0 && firstStage) {
      ...
      await getTenantDb().deal.updateMany({
...
export async function getContactById(id: string) {
  ...
  await AuthorizationService.checkResourceAccess(session.userId, session.organizationId, "CONTACT", id, "VIEW");

  try {
    const contact = await getTenantDb().contact.findUnique({
    ...
    const activityLogs = await getTenantDb().activityLog.findMany({
      where: {
        organizationId: session.organizationId,
...
async function findCompanyByTaxCode(organizationId: string, taxCode: string) {
  ...
  return getTenantDb().company.findFirst({
    where: {
      organizationId,
      taxCode: {
        equals: taxCode,
...
async function resolveCompanyId(organizationId: string, payload: ReturnType<typeof normalizeContactPayload>) {
  ...
  if (payload.companyId) {
    const existing = await getTenantDb().company.findFirst({
  ...
  if (!existing && payload.companyEmail) {
    existing = await getTenantDb().company.findFirst({
      where: {
        organizationId,
        email: {
  ...
  if (existing) {
    const currentCompany = await getTenantDb().company.findUnique({ where: { id: existing.id } });
  ...
  const company = await getTenantDb().company.create({
    data: {
      organizationId,
      ...companyData,
    },
...
export async function createContact(data: ContactPayload) {
  ...
  try {
    ...
    const contact = await getTenantDb().contact.create({
      data: {
        ...contactData,
        companyId,
        organizationId: session.organizationId,
    ...
    // Tự động cấp quyền FULL cho người tạo
    await getTenantDb().resourceAccess.create({
      data: {
        organizationId: session.organizationId,
...
export async function updateContact(id: string, data: ContactPayload) {
  ...
  try {
    ...
    const result = await getTenantDb().contact.updateMany({
      where: { id, organizationId: session.organizationId },
      data: {
        ...contactData,
        companyId,
...
export async function sendPortalAccessEmailForContact(contactId: string, password?: string) {
  ...
  const existingUsers = await getTenantDb().user.findMany({
  ...
  await getTenantDb().account.upsert({
    where: {
...
export async function getCompanyById(id: string) {
  const session = await requireCrmSession();
  try {
    const company = await getTenantDb().company.findFirst({
      where: { id, organizationId: session.organizationId },
      include: {
        contacts: true,
        deals: true
...
export async function updateCompany(id: string, data: Partial<{
  ...
  const session = await requireCrmSession();
  try {
    await getTenantDb().company.updateMany({
      where: { id, organizationId: session.organizationId },
      data,
    });
    return { success: true };
