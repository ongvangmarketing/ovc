...
export default async function PrintDocumentPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  doc = await getTenantDb().quotation.findUnique({ where: { token }, include: includeDocumentData });
  if (doc) docType = "quotation";
  else {
    doc = await getTenantDb().contract.findUnique({ where: { token }, include: includeDocumentData });
    if (doc) docType = "contract";
    else {
      doc = await getTenantDb().invoice.findUnique({ where: { token }, include: includeDocumentData });
