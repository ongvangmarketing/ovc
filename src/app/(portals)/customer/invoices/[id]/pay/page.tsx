...
import { getCustomerPortalData } from "@/app/(portals)/customer/portal-data";
...
export default async function InvoicePaymentPage({
  ...
  // Fetch portal data which handles all permissions automatically
  const portalData = await getCustomerPortalData();
  
  // Find the exact invoice from the authorized list
  const authorizedInvoice = portalData.invoices?.find(inv => inv.id === invoiceId);

  if (!authorizedInvoice) return notFound();

  // Now safely fetch the full invoice with items
  const invoice = await CustomerPortalService.getInvoiceDetails(invoiceId);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#fafafa] p-6 font-sans">
        <div className="w-full max-w-md bg-white rounded-2xl border border-[#eaeaea] p-8 text-center shadow-sm">
          ...
          <Link href="/customer/finance" className="mt-8 inline-flex items-center justify-center rounded-full bg-black px-6 py-2.5 text-[14px] font-medium text-white transition-colors hover:bg-gray-800">
            Về trang Quản lý Tài chính
          </Link>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#fafafa] text-black font-sans selection:bg-black selection:text-white">
      ...
      <div className="border-b border-[#eaeaea] bg-white px-6 h-16 flex items-center sticky top-0 z-40">
        <div className="w-full max-w-[1200px] mx-auto flex items-center gap-4">
          <Link href="/customer" className="flex items-center justify-center h-8 w-8 rounded-full border border-[#eaeaea] text-gray-500 hover:text-black hover:bg-gray-50 transition-colors">
