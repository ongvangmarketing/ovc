...
export default function TourInventoryCalendar({ tour }: { tour: Tour }) {
  ...
  const fetchInventory = async () => {
    ...
    if (res.success && 'inventory' in res && res.inventory) {
      const rawInventory = (res as any).inventory as any[];
      const normalized = rawInventory.map((inv: any) => ({
        ...inv,
            ...
            <tr>
              ...
              {dates.map((d, i) => {
                ...
                return (
                  ...
                  >
                    <div className={`flex flex-col h-full rounded-md p-2 ${isSelected ? 'ring-1 ring-blue-400' : ''} ${cell.allotment === 0 && !cell.isClosed ? 'bg-orange-50/50' : ''}`}>
                      <div className="flex flex-wrap gap-1 mb-1.5">
                        {cell.isClosed ? (
                          <span className="text-[10px] font-medium text-red-600 bg-red-100 rounded px-1.5 w-max">Đóng</span>
                        ) : cell.allotment === 0 ? (
                          <span className="text-[10px] font-medium text-orange-600 bg-orange-100 rounded px-1.5 w-max" title="Chưa nhập số lượng khách">Trống quỹ chỗ</span>
                        ) : (
