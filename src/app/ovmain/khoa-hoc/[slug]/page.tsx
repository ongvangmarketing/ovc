...
export default async function OvCourseDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const org = await getOrganization();
  if (!org) notFound();
  const course = await getCourseBySlug(org.id, slug);
  ...
  const nextClass = course.classes.find((item) => item.startDate) ?? course.classes[0];
