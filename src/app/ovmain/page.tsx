...
import { OvCoursesCarousel } from "./_components/OvCoursesCarousel";
...
function OvServiceCard({ service, index }: { service: { name: string; description: string | null; options?: { id: string; name: string; price: unknown; currency: string; durationText: string | null; featuresJson: unknown }[] }, index: number }) {
  const icons = ["✦", "↗", "◎", "◇", "⌁", "◌"];
  return (
...
function OvPlanningCard({ service }: { service: { name: string; description: string | null; options?: { id: string; name: string; price: unknown; currency: string }[] } }) {
  const optionCopy = [
    {
...
// ─── PAGE ─────────────────────────────────────────────────────────
export default async function OvMainPage() {
    ...
    <>
      ...
      <section className="relative min-h-screen overflow-hidden bg-[#EEF2FF] flex items-center">
        ...
        <div className="hidden 2xl:flex flex-col gap-4 absolute left-8 top-1/3 z-10">
          <OvReveal direction="left" delay={0.6}>
            <StatCard label="Khách hàng" value="500+" sub="+18.5% tháng này" />
          </OvReveal>
          <OvReveal direction="left" delay={0.75}>
        ...
        <div className="relative z-10 mx-auto max-w-7xl w-full px-6 pt-36 pb-28">
          <div className="max-w-5xl">
            ...
            <h1 className="ovmain-hero-title font-black leading-[0.88] tracking-[0] text-slate-900">
              <OvReveal>
        ...
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-4">
            {[
              ...
              <OvReveal key={s.l} delay={i * 0.08}>
                <div className="text-center">
                  <p className="ovmain-stat-value font-black bg-gradient-to-r from-amber-500 to-orange-600 bg-clip-text text-transparent">{s.v}</p>
                  <p className="ovmain-stat-label mt-3 font-semibold text-slate-500">{s.l}</p>
                </div>
              </OvReveal>
            ))}
          </div>
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-6">
          <div className="grid min-w-0 gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
            <div className="min-w-0 max-w-full lg:max-w-2xl">
              ...
              <OvReveal delay={0.1}>
                <h2 className="ovmain-feature-title font-black tracking-[0] text-slate-900">
            ...
            <OvReveal direction="left" delay={0.15} className="min-w-0">
              <div className="relative w-full max-w-full overflow-hidden rounded-2xl border border-white bg-white shadow-2xl shadow-orange-100/70">
                ...
                <div className="ovmain-floating-badge absolute left-4 top-4 flex items-center gap-2 rounded-full border border-white/70 bg-white/92 px-3 py-2 text-[10px] font-black uppercase tracking-[0.08em] text-slate-800 shadow-xl backdrop-blur sm:left-5 sm:top-5 sm:px-4 sm:text-xs">
                  <Lightbulb className="h-4 w-4 text-orange-500" />
                  Sáng tạo
                </div>
                <div className="ovmain-floating-badge ovmain-floating-badge-delay-1 absolute right-4 top-4 flex items-center gap-2 rounded-full border border-white/70 bg-white/92 px-3 py-2 text-[10px] font-black uppercase tracking-[0.08em] text-slate-800 shadow-xl backdrop-blur sm:right-5 sm:top-8 sm:px-4 sm:text-xs">
                  <Target className="h-4 w-4 text-orange-500" />
                  Chiến lược
                </div>
                <div className="ovmain-floating-badge ovmain-floating-badge-delay-2 absolute right-6 top-[42%] hidden items-center gap-2 rounded-full border border-white/70 bg-white/92 px-4 py-2 text-xs font-black uppercase tracking-[0.12em] text-slate-800 shadow-xl backdrop-blur sm:flex">
                  <BarChart3 className="h-4 w-4 text-orange-500" />
                  +35% tăng trưởng
                </div>
                <div className="ovmain-floating-badge ovmain-floating-badge-delay-3 absolute left-6 top-[50%] hidden items-center gap-2 rounded-full border border-white/70 bg-white/92 px-4 py-2 text-xs font-black uppercase tracking-[0.12em] text-slate-800 shadow-xl backdrop-blur sm:flex">
      ...
      {featuredServices.length > 0 && (
        <section id="services" className="relative scroll-mt-32 overflow-hidden bg-white py-24">
          ...
          <div className="pointer-events-none absolute -top-2 right-0 left-[28%] hidden justify-end overflow-hidden select-none lg:flex" aria-hidden="true">
            <span className="ovmain-solutions-ghost ovmain-services-ghost whitespace-nowrap text-[13vw] font-black uppercase leading-none" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
              SOLUTIONS
            </span>
          </div>
          <div className="relative z-10 mx-auto max-w-7xl px-6">
            <div className="mb-12 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <OvReveal><p className="mb-2 text-sm font-bold uppercase tracking-widest text-orange-500">Giải pháp của chúng tôi</p></OvReveal>
                <OvReveal delay={0.1}>
                  <h2 className="ovmain-solutions-title font-black tracking-[0] text-slate-950">
            ...
            {planningService ? <OvPlanningCard service={{ ...planningService, options: planningService.options.map((option) => ({ ...option, price: Number(option.price) })) }} /> : null}
            <OvHorizontalCarousel>
              {carouselServices.map((service, i) => (
                <OvReveal key={service.id} delay={i * 0.08} className="w-[86vw] max-w-[390px] shrink-0 snap-start sm:w-[calc((100%_-_20px)/2)] lg:w-[calc((100%_-_40px)/3)] lg:max-w-none">
                  <OvServiceCard
                    service={{
                      name: service.name,
      ...
      {courses.length > 0 && (
        <section id="training" className="relative scroll-mt-32 overflow-hidden bg-[#f8f6f2] py-24">
          ...
          <div className="relative z-10 mx-auto max-w-7xl px-6">
            ...
            <OvCoursesCarousel courses={courses.map((course) => ({ ...course, price: Number(course.price) }))} />
            <OvReveal delay={0.15}>
