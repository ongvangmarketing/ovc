...
import { getTrainingClassDetail } from "@/lib/training";
...
type TrainingClassDetail = NonNullable<Awaited<ReturnType<typeof getTrainingClassDetail>>>;
...
export default async function ClassDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const item = (await getTrainingClassDetail(id).catch(() => null)) ?? getMockClassDetail(id);
  if (!item) notFound();
    ...
    <div className="page-container mx-auto max-w-[1500px] space-y-5">
      <header className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex min-w-0 items-center gap-4">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <h1 className="truncate text-xl font-bold text-slate-950">{item.name}</h1>
                <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${item.isActive ? "bg-emerald-50 text-emerald-600" : "bg-slate-100 text-slate-500"}`}>{classStatusLabel(item.isActive)}</span>
              </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {!isMockClass ? (
              <Link href={`/workspace/training/classes/${item.id}/edit`} className="inline-flex h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-600 transition hover:bg-slate-50">
                <Pencil className="h-4 w-4" /> Chỉnh sửa
              </Link>
            ) : null}
            <Link href="/workspace/training/tuition/create" className="inline-flex h-10 items-center gap-2 rounded-xl bg-slate-950 px-4 text-sm font-semibold text-white transition hover:bg-slate-800">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
        <main className="space-y-5">
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-base font-bold text-slate-900">Tiến trình lớp học</h2>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-base font-bold text-slate-900">Học viên trong lớp</h2>
                <p className="mt-1 text-sm text-slate-500">{enrollments.length} học viên đang được quản lý trong lớp này.</p>
              </div>
              <Link href="/workspace/training/tuition/create" className="inline-flex h-9 items-center gap-2 rounded-xl border border-orange-200 bg-orange-50 px-3 text-sm font-semibold text-orange-600 transition hover:bg-orange-100">
                  ...
                  {enrollments.length ? enrollments.map((enrollment, index) => (
                    <tr key={enrollment.id} className="transition hover:bg-orange-50/40">
                      ...
                      <td className="px-4 py-4">
                        <Link href={`/workspace/training/tuition/${enrollment.id}`} className="font-bold text-slate-950 hover:text-orange-600">{enrollment.student.name}</Link>
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center justify-between text-xs font-semibold text-slate-500"><span>{enrollment.progress}%</span></div>
                      ...
                      <td className="px-4 py-4 text-right font-bold text-slate-950">{formatMoney(enrollment.tuitionFee)}</td>
                    </tr>
                  )) : (
          ...
          <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="border-b border-slate-200 p-5">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <h2 className="text-base font-bold text-slate-900">Lịch học chi tiết</h2>
                  <p className="mt-1 text-sm text-slate-500">Theo dõi lịch học riêng của lớp {item.name}.</p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="rounded-full bg-indigo-500 px-4 py-2 text-sm font-semibold text-white">Tất cả lịch</span>
                  <span className="rounded-full border border-indigo-200 bg-white px-4 py-2 text-sm font-semibold text-indigo-600">Lịch theo Lớp</span>
                  <span className="rounded-full border border-indigo-200 bg-white px-4 py-2 text-sm font-semibold text-indigo-600">Lịch theo Buổi</span>
                </div>
              </div>
            </div>
            ...
            <div className="border-b border-slate-200 p-5">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div className="flex items-center gap-3">
                  <h3 className="text-lg font-bold capitalize text-slate-950">{scheduleMonth}</h3>
                ...
                {lessonSchedules.length ? lessonSchedules.map((schedule) => (
                  <article key={schedule.id} className="grid gap-4 rounded-2xl border border-slate-200 bg-white p-4 transition hover:border-indigo-200 hover:shadow-sm md:grid-cols-[92px_minmax(0,1fr)_260px] md:items-center">
                    <div className="flex h-20 w-20 flex-col items-center justify-center rounded-2xl bg-slate-50 text-center">
                      <span className="text-xs font-bold uppercase text-slate-500">{formatWeekday(schedule.startsAt)}</span>
                      <span className="text-2xl font-bold text-slate-950">{formatDay(schedule.startsAt)}</span>
                    </div>
                    <div className="min-w-0">
                      ...
                      <h3 className="truncate text-base font-bold text-slate-950">{schedule.title || item.name}</h3>
                  ...
                  <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-10 text-center">
                    ...
                    <p className="mt-3 text-sm font-semibold text-slate-500">Chưa có lịch học cho lớp này.</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <aside className="space-y-5">
          ...
          <SideCard title="Tài liệu lớp học">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-8 text-center">
              ...
              <p className="mt-3 font-bold text-slate-950">{item.code || item.name}</p>
              <p className="mt-1 text-sm text-slate-500">{detail.course.title}</p>
            </div>
            <button type="button" className="mt-3 flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-slate-200 text-sm font-semibold text-slate-600 transition hover:bg-slate-50">
...
function InfoBlock({ label, value, detail, icon, isLast = false }: { label: string; value: string; detail: string; icon: React.ReactNode; isLast?: boolean }) {
  return (
    <div className={`flex gap-4 p-5 ${isLast ? "" : "border-b border-slate-200 md:border-r xl:border-b-0"}`}>
...
function ProgressStep({ label, active }: { label: string; active: boolean }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <span className={`text-sm font-semibold ${active ? "text-slate-700" : "text-slate-400"}`}>{label}</span>
    </div>
  );
}
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${tone}`}>{label[status] || status}</span>;
}
...
function SideRow({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-4 py-2 text-sm">
      ...
      <span className={`text-right ${strong ? "font-bold text-slate-950" : "font-semibold text-slate-700"}`}>{value}</span>
    </div>
  );
}
...
function ActionLink({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
  return (
    <Link href={href} className="mb-3 flex h-11 w-full items-center justify-center gap-2 rounded-xl border border-slate-200 text-sm font-semibold text-slate-600 transition hover:bg-slate-50">
      {icon} {label}
    </Link>
  );
}
