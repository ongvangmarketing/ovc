...
import { createProject, uploadProjectThumbnail } from "@/app/actions/projects";
...
export function ProjectsClient({ initialProjects }: { initialProjects: ProjectLite[] }) {
  ...
  return (
    <div className="flex min-h-[calc(100vh-64px)] flex-col bg-white">
      ...
      <div className="max-w-[1440px] mx-auto w-full px-6 py-10 lg:px-12">
        
        {/* Top Header */}
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-8 mb-16">
          <div>
            <h1 className="text-[36px] font-medium tracking-tighter text-black leading-none mb-3">
              Quản lý Dự án
            </h1>
          ...
          <div className="flex items-center gap-8">
            <div className="flex flex-col gap-2">
              <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Tổng dự án</span>
              <span className="text-[28px] font-medium text-black leading-none tracking-tighter">{initialProjects.length}</span>
            </div>
            <div className="w-[1px] h-12 bg-[#eaeaea]"></div>
            <div className="flex flex-col gap-2">
              <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Đang triển khai</span>
              <span className="text-[28px] font-medium text-black leading-none tracking-tighter">{activeCount}</span>
            </div>
            <div className="w-[1px] h-12 bg-[#eaeaea] hidden sm:block"></div>
            <button onClick={() => setIsModalOpen(true)} className="hidden sm:inline-flex h-10 items-center justify-center rounded-md bg-black px-5 text-[14px] font-medium text-white transition-colors hover:bg-gray-800">
        ...
        <div className="w-full h-[1px] bg-[#eaeaea] mb-12"></div>
          ...
          <div className="flex items-center gap-4">
            <div className="relative w-[200px] sm:w-[260px]">
              ...
              <input 
                ...
                className="h-10 w-full rounded-md border border-[#eaeaea] bg-white py-2 pl-9 pr-4 text-[14px] text-black outline-none transition-colors focus:border-black focus:ring-1 focus:ring-black placeholder:text-gray-400" 
              />
            </div>
            <div className="flex items-center rounded-md border border-[#eaeaea] bg-white p-0.5">
              <button 
                onClick={() => setViewMode("list")} 
                className={cn("p-1.5 rounded-[4px] flex items-center justify-center transition-all", viewMode === "list" ? "bg-gray-100 text-black" : "text-gray-400 hover:text-black")}
              >
                <ListIcon className="w-4 h-4" strokeWidth={2} />
              </button>
              <button 
                onClick={() => setViewMode("grid")} 
                className={cn("p-1.5 rounded-[4px] flex items-center justify-center transition-all", viewMode === "grid" ? "bg-gray-100 text-black" : "text-gray-400 hover:text-black")}
              >
...
function ProjectStrictCard({ project, viewMode }: { project: ProjectLite, viewMode: "grid" | "list" }) {
  ...
  return (
    <Link href={`/workspace/projects/${project.id}`} className={cn(
      "group flex flex-col justify-between rounded-2xl border border-[#eaeaea] bg-white transition-colors hover:border-gray-300 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-black overflow-hidden",
