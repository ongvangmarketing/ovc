...
export async function getInventory(tourId: string, startDate: string, endDate: string) {
  try {
    ...
    return await TourService.getInventory(authData.organizationId, tourId, startDate, endDate);
  } catch (error: any) {
...
export async function bulkUpdateInventory(
  ...
  try {
    ...
    revalidatePath(`/workspace/traveling/tours/${tourId}/schedule`);
    return { success: true };
  } catch (error: any) {
    return { success: false, error: error.message };
  }
...
export async function advancedBulkUpdate(
  ...
  try {
    ...
    await TourService.advancedBulkUpdate(
