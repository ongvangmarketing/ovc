...
export async function updateHotelInfo(formData: FormData) {
  try {
    ...
    revalidatePath(`/workspace/traveling/hotels/${hotelId}`);
    return { success: true };
  } catch (error: any) {
...
export async function createRoomType(formData: FormData) {
  try {
    ...
    revalidatePath(`/workspace/traveling/hotels/${hotelId}/room-types`);
...
export async function updateRoomInventoryBatch(payload: {
  ...
  try {
    ...
    revalidatePath(`/workspace/traveling/hotels/${payload.hotelId}/schedule`);
    return result;
  } catch (error: any) {
    return { success: false, error: error.message };
  }
