...
export default async function HotelDashboardPage({
  ...
  const hotel = await HotelService.getHotelDashboard(authData.organizationId, id);

  if (!hotel) {
    notFound();
  }

  // Dashboard logic and metrics calculations
  const pendingBookings = hotel.bookings.length;
    ...
    <div className="w-full space-y-8 p-4">
      ...
      {pendingBookings > 0 && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 flex items-start gap-3">
          ...
          <Link
            href={`/workspace/traveling/hotels/${id}/bookings`}
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Link
            href={`/workspace/traveling/hotels/${id}/schedule`}
