...
import { updateProject, updateProjectSocialReportSetup, uploadProjectThumbnail, generateProjectShareToken, revokeProjectShareToken } from "@/modules/projects/actions/project.actions";
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#fafafa]">
      <div className="mx-auto w-full max-w-[1200px] px-4 py-8 sm:px-8 sm:py-12">
        <div className="mb-10 flex flex-col md:flex-row md:items-start justify-between gap-6 border-b border-[#eaeaea] pb-8">
          <div>
            <div className="flex items-center gap-3 mb-6">
              <Link href={`/workspace/projects/${project.id}`} className="text-gray-400 hover:text-black transition-colors flex items-center gap-1 text-[13px] font-medium">
            ...
            <h1 className="text-[32px] md:text-[44px] tracking-tight leading-[1.15] font-medium uppercase text-black">
              Sửa dự án
            </h1>
            <p className="mt-2 text-[15px] text-gray-500 max-w-xl">
              Cập nhật thông tin, trạng thái và thiết lập báo cáo.
            </p>
          </div>
          
          <div className="flex flex-wrap items-center gap-3 mt-4 md:mt-0">
            <Link href={`/workspace/projects/${project.id}`} className="inline-flex h-9 items-center justify-center rounded-md border border-[#eaeaea] bg-white px-4 text-[13px] font-medium text-black transition-colors hover:bg-gray-50">Hủy</Link>
            <button type="submit" form="edit-project-form" disabled={isSaving} className="inline-flex h-9 items-center justify-center rounded-md bg-black px-4 text-[13px] font-medium text-white transition-colors hover:bg-gray-800 disabled:opacity-60">
        ...
        <form id="edit-project-form" onSubmit={handleSubmit} className="grid gap-8 lg:grid-cols-[1fr_380px]">
            ...
            <section className="rounded-xl border border-[#eaeaea] bg-white p-6 sm:p-8">
              ...
              <div className="grid gap-5 md:grid-cols-2">
                <Field label="Tên dự án" className="md:col-span-2">
                  <input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} className="h-9 w-full rounded-md border border-[#eaeaea] bg-white px-3 py-1.5 text-[13px] text-black outline-none transition-colors hover:bg-gray-50 focus:border-black focus:ring-1 focus:ring-black placeholder:text-gray-400" />
                </Field>
                <Field label="Trạng thái">
                  <select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as ProjectForm["status"] })} className="h-9 w-full rounded-md border border-[#eaeaea] bg-white px-3 py-1.5 text-[13px] text-black outline-none transition-colors hover:bg-gray-50 focus:border-black focus:ring-1 focus:ring-black">
                    {statuses.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}
                  </select>
                </Field>
                <Field label="Ưu tiên">
                  <select value={form.priority} onChange={(event) => setForm({ ...form, priority: event.target.value as ProjectForm["priority"] })} className="h-9 w-full rounded-md border border-[#eaeaea] bg-white px-3 py-1.5 text-[13px] text-black outline-none transition-colors hover:bg-gray-50 focus:border-black focus:ring-1 focus:ring-black">
                    {priorities.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}
                  </select>
                </Field>
                <Field label="Ngày bắt đầu">
                  <input type="date" value={form.startDate} onChange={(event) => setForm({ ...form, startDate: event.target.value })} className="h-9 w-full rounded-md border border-[#eaeaea] bg-white px-3 py-1.5 text-[13px] text-black outline-none transition-colors hover:bg-gray-50 focus:border-black focus:ring-1 focus:ring-black" />
                </Field>
                <Field label="Hạn hoàn thành">
                  <input type="date" value={form.dueDate} onChange={(event) => setForm({ ...form, dueDate: event.target.value })} className="h-9 w-full rounded-md border border-[#eaeaea] bg-white px-3 py-1.5 text-[13px] text-black outline-none transition-colors hover:bg-gray-50 focus:border-black focus:ring-1 focus:ring-black" />
                </Field>
                <Field label="Ngân sách" className="md:col-span-2">
                  <input type="number" min={0} value={form.budget} onChange={(event) => setForm({ ...form, budget: event.target.value })} className="h-9 w-full rounded-md border border-[#eaeaea] bg-white px-3 py-1.5 text-[13px] text-black outline-none transition-colors hover:bg-gray-50 focus:border-black focus:ring-1 focus:ring-black placeholder:text-gray-400" placeholder="VD: 50000000" />
                </Field>
            ...
            {project.socialMarketingEnabled ? (
              <section className="rounded-xl border border-[#eaeaea] bg-white p-6 sm:p-8">
                ...
                <div className="space-y-4">
                  <div className="rounded-md border border-[#eaeaea] bg-gray-50 p-4">
          ...
          <div className="space-y-8">
            ...
            <section className="rounded-xl border border-[#eaeaea] bg-white overflow-hidden p-6 sm:p-8">
...
function Field({ label, children, className }: { label: string; children: React.ReactNode; className?: string }) {
  return (
    <label className={cn("block", className)}>
      <span className="mb-2 block text-[13px] font-medium text-gray-700">{label}</span>
      {children}
    </label>
  );
}
